...
namespace FTPApp.Models.Utilities
{
    public class FTP
    {
        ...
        public static List<string> GetFilesinDirectory(string url, string username = Constants.FTP.UserName, string password = Constants.FTP.Password)
        {
            List<string> output = new List<string>();
            ...
            using (FtpWebResponse response = (FtpWebResponse)request.GetResponse())
            {
                ...
                using (Stream responseStream = response.GetResponseStream())
                {
                    ...
                    using (StreamReader reader = new StreamReader(responseStream))
                    {
                        ...
                        //Split the response by Carriage Return and Line Feed character to return a list of directories
                        List<string> temp = responseString.Split("\r\n", StringSplitOptions.RemoveEmptyEntries).ToList();

                        foreach (var element in temp)
                        {
                            if (element.Split(" ", StringSplitOptions.RemoveEmptyEntries).ToList()[2] != "<DIR>")
                            {
                                output.Add(element);
                            }
                        }
